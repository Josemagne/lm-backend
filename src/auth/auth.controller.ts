...
const login = async (req: Request, res: Response) => {
  ...
  if (!email || !password) {
    res.status(400).json({
      reason: "Please provide the email and password",
      result: "failure",
    });
  }
  ...
  if (!user) {
    return res
      .status(404)
      .json({ result: "failure", reason: "User does not exist." });
  }
  ...
  return res
    .status(401)
    .json({ result: "failure", reason: "E-Mail or password is false" });
};
...
async function register(req: Request, res: Response) {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .json({ reason: "Please provide email and password", result: "failure" });
  }
  ...
  if (isEmail) user.user_id = nanoid();
    ...
    return res
      .status(401)
      .json({ reason: "E-Mail is not an E-Mail.", result: "failure" });
  ...
  if (passwordIsValid) {
    ...
    if (oldUser) {
      return res
        .status(409)
        .json({ reason: "User already exists", result: "failure" });
    }
  } else
    return res
      .status(401)
      .json({ reason: "Password is corrupt", result: "failure" });
  ...
  await getRepository(User)
    ...
    .catch((err) => {
      ...
      return res
        .status(500)
        .json({ reason: "Could not add user.", result: "failure" });
    });
